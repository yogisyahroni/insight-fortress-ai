...
export default function UploadData() {
  const [uploadedFiles, setUploadedFiles] = useState<{ file: File; status: 'pending' | 'processing' | 'success' | 'error' }[]>([]);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (let i = 0; i < newFiles.length; i++) {
      ...
      setUploadedFiles(prev => 
        prev.map((f, idx) => idx === fileIndex ? { ...f, status: 'processing' } : f)
      );

      const success = await processFile(newFiles[i].file);
      
      setUploadedFiles(prev =>
        prev.map((f, idx) => idx === fileIndex ? { ...f, status: success ? 'success' : 'error' } : f)
      );
  ...
  }, [uploadedFiles.length, processFile, toast]);
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Import your data files for analysis
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <p className="text-muted-foreground mb-4">
          After uploading, you can use our ETL pipeline to:
        </p>
